...
void CShaderWnd::UpdateMatrix()
{
	...
	m_mProjection = perspective<float>(30, m_nXRes/(float)m_nYRes, m_fNear, m_fFar)* lookAt(m_vEye, m_vLookat, m_vUp);

	m_mView = m_mRotation *m_mView;
	m_mRotation = mat4(1.f);
	m_mPVM = m_mProjection * mTranslate* m_mView * m_mModel;
...
void CShaderWnd::Motion(vec3& vVec)
{
	...
	case TRACKBALL_ROTATE:
		...
		angle = atan2(length(axis),dot(m_vDown,vVec));
		axis /= length(axis);
